...
func (nc *NoteCreate) SetConnectionID(u uuid.UUID) *NoteCreate {
	nc.mutation.SetConnectionID(u)
	return nc
}
...
func (nc *NoteCreate) SetConnection(c *Connection) *NoteCreate {
...
func (nc *NoteCreate) check() error {
	if _, ok := nc.mutation.ConnectionID(); !ok {
		return &ValidationError{Name: "connection_id", err: errors.New(`ent: missing required field "Note.connection_id"`)}
	}
	...
	if len(nc.mutation.ConnectionIDs()) == 0 {
		return &ValidationError{Name: "connection", err: errors.New(`ent: missing required edge "Note.connection"`)}
	}
	return nil
}
