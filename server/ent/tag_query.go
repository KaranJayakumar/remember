...
func (tq *TagQuery) QueryConnections() *ConnectionQuery {
	query := (&ConnectionClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withConnections: tq.withConnections.Clone(),
...
func (tq *TagQuery) WithConnections(opts ...func(*ConnectionQuery)) *TagQuery {
	query := (&ConnectionClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withConnections = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withConnections; query != nil {
		if err := tq.loadConnections(ctx, query, nodes, nil,
			func(n *Tag, e *Connection) { n.Edges.Connections = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadConnections(ctx context.Context, query *ConnectionQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Connection)) error {
...
func (tq *TagQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withConnections != nil {
			_spec.Node.AddColumnOnce(tag.FieldConnectionID)
		}
	}
