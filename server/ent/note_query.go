...
func (nq *NoteQuery) QueryConnection() *ConnectionQuery {
...
func (nq *NoteQuery) WithConnection(opts ...func(*ConnectionQuery)) *NoteQuery {
	query := (&ConnectionClient{config: nq.config}).Query()
...
//
//	client.Note.Query().
//		GroupBy(note.FieldConnectionID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (nq *NoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Note, error) {
	...
	if query := nq.withConnection; query != nil {
		if err := nq.loadConnection(ctx, query, nodes, nil,
			func(n *Note, e *Connection) { n.Edges.Connection = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NoteQuery) loadConnection(ctx context.Context, query *ConnectionQuery, nodes []*Note, init func(*Note), assign func(*Note, *Connection)) error {
...
func (nq *NoteQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := nq.ctx.Fields; len(fields) > 0 {
		...
		if nq.withConnection != nil {
			_spec.Node.AddColumnOnce(note.FieldConnectionID)
		}
	}
