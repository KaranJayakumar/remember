...
func (tc *TagCreate) SetConnectionID(u uuid.UUID) *TagCreate {
	tc.mutation.SetConnectionID(u)
	return tc
}

// SetConnectionsID sets the "connections" edge to the Connection entity by ID.
func (tc *TagCreate) SetConnectionsID(id uuid.UUID) *TagCreate {
	tc.mutation.SetConnectionsID(id)
	return tc
}

// SetConnections sets the "connections" edge to the Connection entity.
func (tc *TagCreate) SetConnections(c *Connection) *TagCreate {
	return tc.SetConnectionsID(c.ID)
}
...
func (tc *TagCreate) check() error {
	...
	if _, ok := tc.mutation.ConnectionID(); !ok {
		return &ValidationError{Name: "connection_id", err: errors.New(`ent: missing required field "Tag.connection_id"`)}
	}
	if len(tc.mutation.ConnectionsIDs()) == 0 {
		return &ValidationError{Name: "connections", err: errors.New(`ent: missing required edge "Tag.connections"`)}
	}
	return nil
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.ConnectionsIDs(); len(nodes) > 0 {
