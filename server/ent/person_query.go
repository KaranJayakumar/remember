...
func (pq *PersonQuery) QueryMemories() *MemoryQuery {
	query := (&MemoryClient{config: pq.config}).Query()
...
func (pq *PersonQuery) Clone() *PersonQuery {
	...
	return &PersonQuery{
		...
		withMemories: pq.withMemories.Clone(),
...
func (pq *PersonQuery) WithMemories(opts ...func(*MemoryQuery)) *PersonQuery {
	query := (&MemoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withMemories = query
	return pq
}
...
func (pq *PersonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Person, error) {
	...
	if query := pq.withMemories; query != nil {
		if err := pq.loadMemories(ctx, query, nodes,
			func(n *Person) { n.Edges.Memories = []*Memory{} },
			func(n *Person, e *Memory) { n.Edges.Memories = append(n.Edges.Memories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PersonQuery) loadMemories(ctx context.Context, query *MemoryQuery, nodes []*Person, init func(*Person), assign func(*Person, *Memory)) error {
	...
	query.Where(predicate.Memory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(person.MemoriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.person_memories
		if fk == nil {
			return fmt.Errorf(`foreign-key "person_memories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "person_memories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
