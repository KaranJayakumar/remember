...
func (wq *WorkspaceQuery) QueryConnections() *ConnectionQuery {
	query := (&ConnectionClient{config: wq.config}).Query()
...
func (wq *WorkspaceQuery) Clone() *WorkspaceQuery {
	...
	return &WorkspaceQuery{
		...
		withConnections: wq.withConnections.Clone(),
...
func (wq *WorkspaceQuery) WithConnections(opts ...func(*ConnectionQuery)) *WorkspaceQuery {
	query := (&ConnectionClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withConnections = query
	return wq
}
...
func (wq *WorkspaceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workspace, error) {
	...
	if query := wq.withConnections; query != nil {
		if err := wq.loadConnections(ctx, query, nodes,
			func(n *Workspace) { n.Edges.Connections = []*Connection{} },
			func(n *Workspace, e *Connection) { n.Edges.Connections = append(n.Edges.Connections, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wq *WorkspaceQuery) loadConnections(ctx context.Context, query *ConnectionQuery, nodes []*Workspace, init func(*Workspace), assign func(*Workspace, *Connection)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(connection.FieldWorkspaceID)
	}
	query.Where(predicate.Connection(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workspace.ConnectionsColumn), fks...))
	}))
