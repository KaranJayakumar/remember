...
func (cq *ConnectionQuery) QueryMemories() *MemoryQuery {
	query := (&MemoryClient{config: cq.config}).Query()
...
func (cq *ConnectionQuery) Clone() *ConnectionQuery {
	...
	return &ConnectionQuery{
		...
		withMemories: cq.withMemories.Clone(),
...
func (cq *ConnectionQuery) WithMemories(opts ...func(*MemoryQuery)) *ConnectionQuery {
	query := (&MemoryClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withMemories = query
	return cq
}
...
func (cq *ConnectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Connection, error) {
	...
	if query := cq.withMemories; query != nil {
		if err := cq.loadMemories(ctx, query, nodes,
			func(n *Connection) { n.Edges.Memories = []*Memory{} },
			func(n *Connection, e *Memory) { n.Edges.Memories = append(n.Edges.Memories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConnectionQuery) loadMemories(ctx context.Context, query *MemoryQuery, nodes []*Connection, init func(*Connection), assign func(*Connection, *Memory)) error {
	...
	query.Where(predicate.Memory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(connection.MemoriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.connection_memories
		if fk == nil {
			return fmt.Errorf(`foreign-key "connection_memories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "connection_memories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
