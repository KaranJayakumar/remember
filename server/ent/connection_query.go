...
func (cq *ConnectionQuery) QueryWorkspace() *WorkspaceQuery {
	query := (&WorkspaceClient{config: cq.config}).Query()
...
func (cq *ConnectionQuery) QueryNotes() *NoteQuery {
...
func (cq *ConnectionQuery) QueryTags() *TagQuery {
...
func (cq *ConnectionQuery) Clone() *ConnectionQuery {
	...
	return &ConnectionQuery{
		...
		withWorkspace: cq.withWorkspace.Clone(),
...
func (cq *ConnectionQuery) WithWorkspace(opts ...func(*WorkspaceQuery)) *ConnectionQuery {
	query := (&WorkspaceClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withWorkspace = query
	return cq
}
...
func (cq *ConnectionQuery) WithNotes(opts ...func(*NoteQuery)) *ConnectionQuery {
	query := (&NoteClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *ConnectionQuery) WithTags(opts ...func(*TagQuery)) *ConnectionQuery {
	query := (&TagClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(connection.FieldWorkspaceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Connection.Query().
//		Select(connection.FieldWorkspaceID).
//		Scan(ctx, &v)
...
func (cq *ConnectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Connection, error) {
	...
	if query := cq.withWorkspace; query != nil {
		if err := cq.loadWorkspace(ctx, query, nodes, nil,
			func(n *Connection, e *Workspace) { n.Edges.Workspace = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withNotes; query != nil {
		if err := cq.loadNotes(ctx, query, nodes,
			func(n *Connection) { n.Edges.Notes = []*Note{} },
			func(n *Connection, e *Note) { n.Edges.Notes = append(n.Edges.Notes, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withTags; query != nil {
		if err := cq.loadTags(ctx, query, nodes,
			func(n *Connection) { n.Edges.Tags = []*Tag{} },
			func(n *Connection, e *Tag) { n.Edges.Tags = append(n.Edges.Tags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConnectionQuery) loadWorkspace(ctx context.Context, query *WorkspaceQuery, nodes []*Connection, init func(*Connection), assign func(*Connection, *Workspace)) error {
...
func (cq *ConnectionQuery) loadNotes(ctx context.Context, query *NoteQuery, nodes []*Connection, init func(*Connection), assign func(*Connection, *Note)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(note.FieldConnectionID)
	}
	query.Where(predicate.Note(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(connection.NotesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *ConnectionQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Connection, init func(*Connection), assign func(*Connection, *Tag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Connection)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tag.FieldConnectionID)
	}
	query.Where(predicate.Tag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(connection.TagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *ConnectionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withWorkspace != nil {
			_spec.Node.AddColumnOnce(connection.FieldWorkspaceID)
		}
	}
